...
export default function ProductDetailPage() {
  ...
  const getOptimizedUrl = (url: string, width = 800) => {
    ...
    // f_auto: best format, q_auto: best compression, w_X: specific width
  ...
  const handleChat = () => {
    ...
    navigate(
      `/chat?with=${product.farmer.id}&name=${encodeURIComponent(product.farmer.full_name)}`,
    );
  };
        ...
        <div className="space-y-3">
          <div className="aspect-square bg-green-50 rounded-3xl overflow-hidden">
            {product.photos?.length > 0 ? (
              <img
                src={getOptimizedUrl(product.photos[photoIdx], 800)}
                alt={product.name}
          ...
          {product.photos?.length > 1 && (
            <div className="flex gap-2 flex-wrap">
              {product.photos.map((url: string, i: number) => (
                ...
                >
                  {/* Smaller width (200px) for thumbnails to save bandwidth */}
                  <img
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3 bg-green-50 rounded-xl p-1">
                <button
                  ...
                  className="w-8 h-8 rounded-lg bg-white font-bold text-green-700 shadow-sm hover:bg-green-100 transition-colors"
                >
                  −
                </button>
              ...
              <span className="text-sm text-gray-400 font-semibold">
                = {(product.price * qty).toLocaleString()} XAF
              </span>
            </div>
          )}
