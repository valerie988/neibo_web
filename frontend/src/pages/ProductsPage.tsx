...
const CATEGORIES = ["All", "Vegetables", "Fruits", "Grains", "Dairy", "Meat", "Beverages", "Other"];
    ...
    <div onClick={onClick}
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mt-3">
          <div>
            <span className="text-lg font-black text-green-700">{product.price?.toLocaleString()}</span>
            <span className="text-xs text-gray-400 ml-1">XAF/{product.unit}</span>
          </div>
...
function CreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  const [form, setForm] = useState({ name: "", category: "Vegetables", price: "", unit: "kg", quantity: "", description: "", location: "" });
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          {[
            { label: "Product Name", key: "name",        type: "text",   placeholder: "Roma Tomatoes",           required: true  },
            { label: "Location",     key: "location",    type: "text",   placeholder: "Bafoussam, West Region",  required: true  },
            { label: "Price (XAF)",  key: "price",       type: "number", placeholder: "500",                     required: true  },
            { label: "Quantity",     key: "quantity",    type: "number", placeholder: "100",                     required: true  },
            { label: "Description",  key: "description", type: "text",   placeholder: "Fresh and organic…",      required: false },
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Photos (max 4)</label>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{products.length} items found</p>
        </div>
        {user?.role === "farmer" && (
          ...
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setCategory(c)}
              className={`px-3 py-2 rounded-xl text-xs font-semibold whitespace-nowrap transition-all ${
                category === c ? "bg-green-700 text-white" : "bg-white text-gray-500 border border-gray-200 hover:border-green-300"
              }`}>{c}</button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {products.map((p) => (
            <ProductCard key={p.id} product={p} onClick={() => handleView(p.id)} />
          ))}
        </div>
      )}
